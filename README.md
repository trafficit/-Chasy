...
- при активации кода пользователь ставит галочку «принимаю условия» —
  ...
  в `.env`.
- Цена в окне «О программе» — константа `APP_PRICE` в `frontend/app.js`
  (одно место), формулировки — i18n-ключ `about_price`.
- Платёжных реквизитов (Wise/IBAN) в коде и на сайте нет — они только
  в счёте, который отправляется клиенту после заказа.
